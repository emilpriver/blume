...
A string `accent` applies to both color modes. Pass a `{ light, dark }` object for a different accent per mode, and set `background` / `backgroundDark` to override the page background per mode:
...
Each color takes a named preset or any CSS color.

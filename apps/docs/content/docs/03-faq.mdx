...
1. Save the patch as `patches/oxfmt@0.58.0.patch`:
   ...
   index 1905aa7563e612808426e26f1ed28bebbef3456f..3dcdeeeca1f809939218186889826cf55348bc49 100644
   --- a/dist/markdown-BjyDFyaO.js
   +++ b/dist/markdown-BjyDFyaO.js
   @@ -4830,7 +4830,18 @@ function lu(e, t, r) {
   ...
   +			// linkReference that splits the fence text node, so also look one
   +			// sibling further back when the previous node is a (link) reference.
   +			let oxfmtPrevIsTitledFence = oxfmtFencePrev != null && (oxfmtFencePrev.type === "linkReference" || oxfmtFencePrev.type === "link") && oxfmtIsFence(e.parent && e.parent.children[e.index - 2]);
...
:::warning[Version-pinned] The patch targets a specific oxfmt build — its diff references a file whose name is hashed per release (`dist/markdown-*.js`). When you bump oxfmt, regenerate the patch (e.g. `bun patch oxfmt`) or check whether the upstream fix has landed and the patch is no longer needed.
:::

...
:::tip
Migrating from another docs tool? The [`blume-migrate` skill](/docs/advanced/migrate) maps a source's locale config onto this `i18n` block — for a Mintlify repo, `navigation.languages[]` becomes `locales`, the `default: true` language becomes `defaultLocale`, and translated folders already match Blume's layout.
:::

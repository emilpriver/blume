...
- FontAwesome icon names (from `icons.library: fontawesome`) resolve against Blume's icon set through a synonym map — `shield-halved`, `layer-group`, `arrows-rotate`, `gauge-high`, and many more map to their closest Blume icon, with any library prefix (`fa-`, `fa-solid`) stripped first.
- Components with no Blume equivalent are flagged for manual review — reach for the [OpenAPI reference](/docs/advanced/api-reference) in place of Mintlify's API components.

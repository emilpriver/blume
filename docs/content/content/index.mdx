...
`content.root` in [`blume.config.ts`](/configuration)).
...
- **`.md`** — Markdown for plain prose: GFM, frontmatter, smart punctuation, and
  super/subscript.
- **`.mdx`** — everything `.md` has, plus [components](/content/components) and
  the MDX-only [directives, package installs, and math](/content/syntax).
...
Ordering has several layers — see [Navigation](/content/navigation) for the full
precedence rules.
...
<CardGroup cols={2}>
  ...
  <Card title="Syntax" href="/content/syntax" icon="book-open">
  ...
  <Card title="Navigation" href="/content/navigation" icon="menu">

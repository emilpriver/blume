...
[`_meta.json`](/docs/content/navigation#folder-groups) files resolve per locale,
too: put a `_meta.json` under `fr/guides/` to order the French group
...
:::note
The first release covers the locale-folder layout (the `dir` parser). The
dotted-filename parser (`page.fr.mdx`), shared locale-agnostic files, and full
right-to-left theming are not yet available.
:::
